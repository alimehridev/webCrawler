...
parser.add_argument("-u", "--url", type=str, required=True, help="URL to crawl . [Required]") # Works Prefectly
parser.add_argument("-c", "--cookies", type=str, default="", help="Set cookies if authentication is required, s perate them with ';' . example: xwidth=1920;xheight=1032;sessionid=ed83623d96a51fb1bc")
parser.add_argument("-d", "--depth", type=str, default=2, help="Assign depth of crawl, default=2") # Just full depth works
...
parser.add_argument("-t", "--threads", type=int, default=3, help='Threads number, default=3 and max=16') # Works but can be better
parser.add_argument("-a", "--assets", action="store_true", help='If crawling assets like js files is necessary .')
parser.add_argument("-o", "--output_file", default="[url].txt", help='output_file to save the output . [default=[url].txt]') # Can be removed .
...
parser.add_argument("-co", "--continue", action="store_true", help='Continue last crawl .') # Works but check it 
parser.add_argument("--onlydomain", action="store_true", help='Just save links in this domain and ignore others .') # Works but chekc it 
